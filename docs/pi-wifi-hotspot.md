...
- **First join works, then “Forget” and reconnect stays on “Connecting”** (common on Pi **brcmf** + **hostapd**): the current setup writes **`disassoc_low_ack=0`**, **`ap_max_inactivity=0`**, and (WPA2) **`wpa_group_rekey=0`** into **`/etc/hostapd/hostapd.conf`**. Re-run **`./setuphotspot`** after **`git pull`**. If it still happens, from the Pi run **`sudo systemctl restart hostapd`** (or reboot), or use a **USB Wi-Fi dongle** and **`AP_IFACE=wlan1`**.
...
- **Why `sudo systemctl start NetworkManager` does not bring Wi‑Fi back by itself:** after setup, **`hostapd` is still running** and keeps `wlan0` in **AP mode**. NetworkManager cannot manage a card that is already an access point. You must **stop the hotspot first**, then start NM. One command from the project root: **`sudo ./scripts/restore-client-wifi.sh`**. It kills `hostapd` and `dnsmasq`, drops captive **iptables** rules (bounded in time), flushes the static demo IP, may **`systemctl reset-failed NetworkManager`**, then **queues** **`NetworkManager` with `systemctl start --no-block`** (the script does **not** wait for NM to be fully “active” — a plain `start` on a busy Pi can block for a long time). Wi‑Fi may take **1–2 minutes** to reappear; if not, `sudo systemctl restart NetworkManager` or **`sudo reboot`**. The script also **`disables` `hostapd` and `dnsmasq` on boot** so the hotspot is not re-enabled every reboot. To run the demo again: **`./setuphotspot`**. The setup **`enable`s `hostapd` and `dnsmasq`** for boot — restore **disables** them again.
...
- If the script **stuck on** “Verifying hostapd config” / `hostapd -t` — on some on-board **Broadcom (brcmf) Wi-Fi** the config test can **block forever**; the script time-limits that step (default 15s) and continues, or **skips** the test if `timeout` (from **coreutils**) is missing. In `.hotspot.env` you can set **`HOTSPOT_HOSTAPD_TEST_SEC=1`** to shorten; the real test is the **Start hostapd** step that follows.
